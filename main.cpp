...
class Ship {
	public:
			...
			for (int i = 0; i < length; ++i) {
				...
				switch (direction) {
					...
					case ShipDirection::LEFT:
						this->shipParts.push_back(ShipPart(row, column-i, CellType::SHIP)); // Сама операция добавления
						break;
					// Если направление вниз - то уменьшаем номер строки
					case ShipDirection::UP:
						this->shipParts.push_back(ShipPart(row-i, column, CellType::SHIP));// Сама операция добавления
						break;
		...
		Ship(int row, int col, CellType type) {
			this->shipParts.push_back(ShipPart(row, col, type));
		}
		...
		void addPart(int row, int column, CellType type) {
			this->shipParts.push_back(ShipPart(row, column, type));
		}
	...
	public:
		...
		void applyAttack(int row, int column, CellType type) {
			...
			if (type == CellType::ATTACKED) {
				...
				this->ships.push_back(Ship(row, column, type));
			}
		}
...
class Player : public ControlEntity {
	public:
		...
		pair<int, int> getAttackCell() const {
			...
			return make_pair(cell.first, cell.second);
		}
...
class Bot : public ControlEntity {
	...
	public:
		...
		pair<int, int> getAttackCell() {
			...
			std::pair<int, int> randomCell = this->generateRandomCell();
			// Добавляем её в историю атак
			this->attackHistory.push_back({randomCell.first, randomCell.second});
...
int main() {
	...
	while (true) {
		...
		{ // Запрашиваем ячеку у игрока
			std::pair<int, int> attackCell = player.getAttackCell();
			// Проводим атаку на бота и отображаем результат на карте атак игрока
			player.applyAttackAtAttackMap(attackCell.first, attackCell.second, bot.applyAttackAtShipMap(attackCell.first, attackCell.second));
		}
		{
			 // Запрашиваем ячеку у бота (генерируется случайно)
			std::pair<int, int> attackCell = bot.getAttackCell();
			// Выводим на экран, куда атаковал бот
			std::cout << "\nBot attack at  " << char(attackCell.second+97) << attackCell.first+1 << "\n\n";
			// Проводим атаку на игрока и отображаем результат на карте атак бота
			bot.applyAttackAtAttackMap(attackCell.first, attackCell.second, player.applyAttackAtShipMap(attackCell.first, attackCell.second));
		}
